...
- `pill_pulse` / PIN_61 / PULSE（加 1 脉冲）
- `start` / PIN_81 / K1（运行/暂停；设置模式下用于切换字段/消音）
- `stop` / PIN_80 / K2（进入/退出设置模式；响铃时可消音）
- `bottle_ok` / PIN_79 / K3（闹钟使能：1启用，0关闭并停铃）
- `sw_mode_limit` / PIN_67 / K12（设置对象：0设置当前时间，1设置闹钟时间）
- `sw_auto_move` / PIN_65 / K13（保留输入，不影响功能）
...
- 校时模式下：
  - `sw_mode_limit=0`（校准当前时间）：计时暂停
  - `sw_mode_limit=1`（设置闹钟）：当前时间继续计时
  - `alarm` 以约 1Hz 闪烁作为提示
...
## 闹钟功能
### 1) 启用/关闭闹钟
- `bottle_ok=1` 启用闹钟比较；`bottle_ok=0` 关闭闹钟并立即停止响铃。

### 2) 设置闹钟时间
- 按 `stop` 进入设置模式。
- 将 `sw_mode_limit` 置 1，进入闹钟设置。
- 按 `start` 在“闹钟小时/闹钟分钟”之间切换。
- 被选中的字段会闪烁显示。
- 按 `pill_pulse` 对当前字段加 1（闹钟小时 00~23，闹钟分钟 00~59）。
- 再按 `stop` 退出设置模式。
-
闹钟设置规则：
...
- 闹钟比较发生在 00 秒的计时节拍上。
- 响铃持续约 30 秒，或按 `start` / `stop` 消音，或将 `bottle_ok` 置 0 关闭闹钟。
